        ...
        try
        {
            ...
            if($data) {
                ...
                if($flag == 'sukses') {
                    ...
                    $user = DB::connection(config('app.URL_SQLSRV93_DEV'))->select("EXEC [dbo].[ESSLoginSessionGet]?", [$nik]);
    ...
    public static function Authenticate($request){
        try{
            ...
            $response = $client->post(
                'http://'.config('app.URL_DEV').'/RESTSecurity/RESTSecurity.svc/GetTokenDetail',
                [
                    RequestOptions::JSON => 
                    ['token' => str_contains($request->cookie('Authorization-dev'), 'Bearer') ? substr($request->cookie('Authorization-dev'),6) : $request->cookie('Authorization-dev')]
                ],
