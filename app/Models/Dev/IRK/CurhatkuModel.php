...
class CurhatkuModel extends Model
{
    ...
    public static function showDataCurhatku($request)
    {
        ...
        try
        {
            ...
            $data = DB::connection(config('app.URL_PGSQLGCP_IRK_DEV'))->select("select * from showcurhatkulist(?,?)",[$userid,$page]);
    ...
    public static function showDataCurhatkuSingle($request)
    {
        ...
        try
        {
            $data = DB::connection(config('app.URL_PGSQLGCP_IRK_DEV'))->select("select * from showcurhatkudetail(?,?)",[$userid,$idticket]);
    ...
    //                 $data = DB::connection(config('app.URL_PGSQLGCP_IRK_DEV'))->insert("CALL inputcurhatku(?,?,?,?,?)", [$nik,$caption,$deskripsi,$alias,$imgextension]);
    ...
    //             }
    //         } else{
    //             $data = DB::connection(config('app.URL_PGSQLGCP_IRK_DEV'))->insert("CALL inputcurhatku(?,?,?,?,?)", [$nik,$caption,$deskripsi,$alias,'']);
            ...
            if(!empty($gambar)){
                ...
                if ($gambar->getSize() > 1048576 || !in_array($gambar->extension(), $imgformat)){
                    ...
                    if($data) {
                        ...
                        $imgpath = 'Dev/Ceritakita/Curhatku/'.$idimg.'.'.$imgextension;
                ...
                if($data) {
                    ...
                    $imgpath = 'Dev/Ceritakita/Curhatku/'.$idimg.'.';
