...
class Credential extends Model
{
    ...
    public static function IsTokenSignatureValid(string $token)
    {
		...
		$response = $client->post(
            'http://'.config('app.URL_STAG').'/RESTSecurityStag/RESTSecurity.svc/DecodeTokenStag',
            [
                RequestOptions::JSON => 
                ['token'=>$token]
            ],
	...
	public static function Login($postbody)
    {
		...
		$response = $client->post(
            'http://'.config('app.URL_STAG').'/RESTSecurityStag/RESTSecurity.svc/LoginESSV2',
            [
                RequestOptions::JSON => 
                ['user'=>$postbody]
            ],
    ...
    public static function GetTokenAuth(string $nik)
    {
        ...
        $response = $client->post(
            'http://'.config('app.URL_STAG').'/RESTSecurityStag/RESTSecurity.svc/GetTokenForStag',				
            [
                RequestOptions::JSON => 
                ['nik' => $nik]
            ],
