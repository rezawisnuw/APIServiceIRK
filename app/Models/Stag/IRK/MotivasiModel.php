...
class MotivasiModel extends Model
{
    ...
    public static function showDataMotivasiSingle($request)
    {
        ...
        try
        {
            $data = DB::connection(config('app.URL_PGSQLGCP_IRK_STAG'))->select("select * from showmotivasi(?,?)",[$userid,$idticket]);
        ...
        try
        {
            if(!empty($photo)){
                ...
                if ($photo->getSize() > 1048576 || !in_array($photo->extension(), $imgformat)){
                    ...
                    $data = DB::connection(config('app.URL_PGSQLGCP_IRK_STAG'))->insert("CALL inputmotivasi(?,?,?,?,?)", [$nik,$judul,$motivasi,$alias,$imgextension]);
                    ...
                    if($data) {
                        ...
                        //             [
                        //              'file'=> $photo,
                        //              'file_name' => 'Stag/'.$nik.'_'.$nextId.'.'.$imgextension
                        //             ]
                        //         ]
                        //     );
                        ...
                        $imgpath = 'Stag/Ceritakita/Motivasi/'.$nik.'_'.$nextId.'.'.$imgextension;
                ...
                $data = DB::connection(config('app.URL_PGSQLGCP_IRK_STAG'))->insert("CALL inputmotivasi(?,?,?,?,?)", [$nik,$judul,$motivasi,$alias,'']);
                ...
                if($data) {
                    ...
                    $imgpath = 'Stag/Ceritakita/Motivasi/'.$nik.'_'.$nextId.'.';
