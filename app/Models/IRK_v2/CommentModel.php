    ...
    public function inputDataReplyNewComment($request)
    {
        ...
        try {
            ...
            if ($data) {
                ...
                $this->status = 'Success';
                $this->message = $response->Result->status == 1 ? $response->Result->message : 'Silahkan periksa aktifasi izin notifikasi pada browser anda terlebih dahulu';
                $this->data = $response->Result->status == 1 ? ["blocked" => $target->is_blocked == false, "likedby" => $likedby, "ttllike" => $ttllike, "ttlcomment" => $ttlcomment, "ttlnewcomment" => $ttlnewcomment] : $data;
            } else {
                $this->status;
                $this->message;
                $this->data;
