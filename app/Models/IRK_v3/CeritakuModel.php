...
class CeritakuModel extends Model
{
    ...
    public function showDataCeritaku($request)
    {
        ...
        try {
            $data = [];
            $data = $this->connection->select("select * from public_v3.showceritakulist(?,?)", [$userid, $page]);
    ...
    public function showDataCeritakuSingle($request)
    {
        ...
        try {
            $data = [];
            $data = $this->connection->select("select * from public_v3.showceritakudetail(?,?)", [$userid, $idticket]);
            ...
            if (!empty($gambar)) {
                ...
                foreach ($gambar as $key => $value) {
                    if (!in_array($value->extension(), $imgformat) || $value->getSize() > 1048576) { // in bytes
                        ...
                        $imgpath[] = $this->path . '/Ceritakita/Ceritaku/' . $imgname[$key];
                    }
                }
