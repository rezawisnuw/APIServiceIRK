...
class IdeakuModel extends Model
{
    ...
    public function showDataIdeaku($request)
    {
        ...
        try {
            $data = [];
            $data = $this->connection->select("select * from public_v3.showideakulist(?,?)", [$userid, $page]);
    ...
    public function showDataIdeakuSingle($request)
    {
        ...
        try {
            $data = [];
            $data = $this->connection->select("select * from public_v3.showideakudetail(?,?)", [$userid, $idticket]);
