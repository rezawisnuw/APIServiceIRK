...
class FaqModel extends Model
{
    ...
    public function showDataFaq($request)
    {
        try {
            ...
            $query = $this->connection->table('public_v3.Faq');
    ...
    public function inputDataFaq($request)
    {
        ...
        try {
            $data = $this->connection->insert("CALL public_v3.inputfaq(?,?,?,?,?,?)", [$question, $answer, $category, $nik, $alias, $platform]);
    ...
    public function editDataFaq($request)
    {
        ...
        try {
            $data = $this->connection->insert("CALL public_v3.editfaq(?,?,?,?,?)", [$idfaq, $question, $answer, $category, $nik]);
    ...
    public function deleteDataFaq($request)
    {
        try {
            ...
            if (!is_null($idfaq)) {
                ...
                $data = $this->connection->table('public_v3.Faq')->whereIn('id_faq', $idfaqArray)->delete();
            }
            if ($data) {
                $this->status = 'Success';
